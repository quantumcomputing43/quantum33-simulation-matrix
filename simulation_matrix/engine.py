...
from simulation_matrix.contracts import (
    INFRASTRUCTURE_FAILURE, IMPLEMENTATION_FAILURE, METHOD_FAILURE,
    SCIENTIFIC_FAILURE, RESULT,
)
...
@contextmanager
def _state_lock(checkpoint: Path):
...
def _contained_input(root: Path, path: Path, label: str) -> Path:
    ...
    if root_resolved not in resolved.parents:
        raise ConfigError(f"{label} path escapes root: {path}")
    return resolved
...
def run(root: Path, *, project_id: str, matrix_path: Path | None = None,
        project_manifest_path: Path | None = None) -> dict:
    root = root.resolve()
    validate_project_id(project_id)
    matrix_path = _contained_input(
        root, matrix_path or (root / "simulation_matrix" / "matrix.json"), "matrix"
    )
    project_manifest_path = _contained_input(
        root,
        project_manifest_path or (root / "simulation_matrix" / "projects" / f"{project_id}.json"),
        "project manifest",
    )
    ...
    with _state_lock(checkpoint):
        ...
        if checkpoint.exists():
            try:
                ...
                raise RuntimeError(
                    f"INFRASTRUCTURE_FAILURE: unreadable checkpoint: {type(exc).__name__}"
                ) from exc
        ...
        for spec in matrix["pipeline"]:
            ...
            try:
                ...
                state["status"] = IMPLEMENTATION_FAILURE
            ...
            if not result.passed:
                ...
                state["status"] = (
                    failure if failure in {METHOD_FAILURE, SCIENTIFIC_FAILURE} else METHOD_FAILURE
                )
                _write(latest, state)
                _write(checkpoint, state)
                return state
        ...
        survivor = next(
            (x for x in state["stages"] if x["stage"] == "SURVIVOR_CHECK"), None
        )
        if enabled != done:
