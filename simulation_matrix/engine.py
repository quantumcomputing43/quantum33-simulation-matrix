...
def run(root:Path)->dict:
    ...
    state={"matrix_version":matrix["matrix_version"],"case_id":adapter.case_id,"protocol_version":adapter.protocol_version,
           "seed":matrix["seed"],"scientific_experiment":False,"execution_identity":identity,
           "status":"RUNNING","stages":[],"resumed":False,"timestamp":_now()}
    ...
    for spec in matrix["pipeline"]:
        ...
        try:
            result=adapter.run_stage(stage,seed=matrix["seed"],context={"matrix":matrix,"execution_identity":identity})
            ...
            _event(log,{**item,"timestamp":_now()}); _write(checkpoint,state); _write(latest,state)
            return state
        ...
        if not result.passed:
            ...
            _write(latest,state); _write(checkpoint,state); return state
    ...
    _event(log,{"event":"RUN_COMPLETE","status":state["status"],"timestamp":_now()})
    return state
